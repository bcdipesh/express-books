...
describe("POST /books", function () {
  ...
  test("Prevents creating book without required title", async function () {
    const response = await request(app).post(`/books`).send({ year: 2023 });
...
describe("PUT /books/:id", function () {
  test("Updates a single book", async function () {
    ...
    expect(response.body.book).toHaveProperty("isbn");
    expect(response.body.book.title).toBe("Lorem Ipsum: Dolor Init");
    expect(response.body.book.language).toBe("English");
    expect(response.body.book.pages).toBe(200);
  });
